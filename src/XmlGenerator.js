class XmlGenerator {
  ...
  static generateAdaptationSet(videoFormatArray) {
      ...
      videoFormatArray.forEach((videoFormat) => {
          ...
          let mimeType = videoFormat.mimeType.split(";")[0];
          if (mimeType.includes("video")) {
              const codecs = videoFormat.mimeType
                  .split(" ")[1]
                  .match(/"[^"]*/)[0]
                  .substr(1);

              const format = codecs.split(".")[0];
              mimeType += `;${format}`;
          }
      ...
      for (let i = 0; i < mimeTypes.length; i++) {
          ...
          mimeObjects[i].forEach((format) => {
              if (isVideoFormat) {
                  adapSet.elements.push(
                      this.generateRepresentationVideo(format)
                  );
              } else {
                  adapSet.elements.push(
                      this.generateRepresentationAudio(format)
                  );
              }
          });
                  ...
                  elements: [
                      {
                          ...
                          name: "Period",
                          elements:
                              this.generateAdaptationSet(videoFormatArray),
                      },
                  ],
              },
          ],
